...
public class ReserveStockCommandImpl implements ReserveStockCommand {
    ...
    public StockResponse execute(Request request) {
        ...
        StockMovement stockMovement = createStockMovement(savedStock, reserveRequest, previousReserved, savedStock.getReservedQuantity());

        log.info("Stock {} reserved successfully. Reserved quantity: {}",
                request.stockId(), savedStock.getReservedQuantity());
        StockResponse response = stockMapper.toResponse(savedStock);
        response.setReservationId(stockMovement.getId());
        return response;
    }

    private StockMovement createStockMovement(Stock stock, StockReserveRequest request,
