...
//    }
    ...
    void createVariant_WithNonExistentItem_ShouldReturnNotFound() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/v1/items/" + nonExistentItemId + "/variants")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andDo(print())
    ...
    void updateVariant_WithNonExistingId_ShouldReturnNotFound() throws Exception {
        ...
        // When & Then
        mockMvc.perform(put("/api/v1/variants/" + nonExistingId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateRequest)))
                .andDo(print())
                .andExpect(status().isNotFound())
    ...
    void deleteVariant_WithExistingId_ShouldSoftDeleteVariant() throws Exception {
        ...
        // When & Then
        mockMvc.perform(delete("/api/v1/variants/" + variantId))
                .andDo(print())
                .andExpect(status().isNoContent());
     ...
     */
    private String createTestVariant(String itemId, String variantSku, String variantName, BigDecimal priceAdjustment) throws Exception {
        VariantCreateRequest request = VariantCreateRequest.builder()
                .itemId(itemId)
        ...
        MvcResult result = mockMvc.perform(post("/api/v1/items/" + itemId + "/variants")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated())
                .andReturn();
