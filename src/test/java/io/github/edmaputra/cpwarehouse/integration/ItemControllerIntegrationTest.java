...
class ItemControllerIntegrationTest {
  ...
  void createItem_WithMissingRequiredFields_ShouldReturnBadRequest() throws Exception {
    ...
    ItemCreateRequest request = ItemCreateRequest.builder().sku("TEST-001")
  ...
  void getAllItems_WithMultipleItems_ShouldReturnPagedItems() throws Exception {
    ...
    // When & Then
    mockMvc.perform(get("/api/v1/items").param("size", "10"))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data.content").isArray())
  ...
  void getAllItems_WithIsActiveFilter_ShouldReturnFilteredItems() throws Exception {
    ...
    createTestItem("TEST-001", "Active Item", new BigDecimal("10.00"));
    ...
    mockMvc.perform(get("/api/v1/items").param("isActive", "true"))
        .andDo(print())
        .andExpect(status().isOk())
  ...
  void getAllItems_WithSearchTerm_ShouldReturnMatchingItems() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/items").param("search", "Blue"))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.content", hasSize(2)))
  ...
  void updateItem_WithValidRequest_ShouldReturnUpdatedItem() throws Exception {
    ...
    // When & Then
    mockMvc.perform(put("/api/v1/items/" + itemId).contentType(MediaType.APPLICATION_JSON)
  ...
  void updateItem_WithNonExistingId_ShouldReturnNotFound() throws Exception {
    ...
    // When & Then
    mockMvc.perform(put("/api/v1/items/" + nonExistingId).contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(updateRequest)))
        .andDo(print())
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.success").value(false))
  ...
  void deleteItem_WithExistingId_ShouldSoftDeleteItem() throws Exception {
    ...
    // When & Then
    mockMvc.perform(delete("/api/v1/items/" + itemId)).andDo(print()).andExpect(status().isNoContent());
    ...
    Optional<Item> softDeletedItem = itemRepository.findById(itemId);
    assertThat(softDeletedItem).isPresent();
    assertThat(softDeletedItem.get().getIsActive()).isFalse();
  }
  ...
  void hardDeleteItem_WithExistingId_ShouldPermanentlyDeleteItem() throws Exception {
    ...
    // When & Then
    mockMvc.perform(delete("/api/v1/items/" + itemId + "/permanent")).andDo(print()).andExpect(status().isNoContent());
  ...
  @Test
  void hardDeleteItem_WithNonExistingId_ShouldReturnNotFound() throws Exception {
    // Given
    String nonExistingId = "507f1f77bcf86cd799439011";

    // When & Then
    mockMvc.perform(delete("/api/v1/items/" + nonExistingId + "/permanent"))
        .andDo(print())
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error.code").value("RESOURCE_NOT_FOUND"));
  ...
  @Test
  void getAllItems_WithSortingAsc_ShouldReturnSortedItems() throws Exception {
    // Given - Create items with different names
    createTestItem("TEST-003", "Charlie Item", new BigDecimal("30.00"));
    createTestItem("TEST-001", "Alpha Item", new BigDecimal("10.00"));
    createTestItem("TEST-002", "Beta Item", new BigDecimal("20.00"));
    ...
    mockMvc.perform(get("/api/v1/items")
        ...
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data.content", hasSize(3)))
        .andExpect(jsonPath("$.data.content[0].name").value("Alpha Item"))
        .andExpect(jsonPath("$.data.content[1].name").value("Beta Item"))
        .andExpect(jsonPath("$.data.content[2].name").value("Charlie Item"));
  }

  @Test
  void getAllItems_WithSortingDesc_ShouldReturnSortedItems() throws Exception {
    // Given - Create items with different base prices
    createTestItem("TEST-001", "Item One", new BigDecimal("10.00"));
    createTestItem("TEST-002", "Item Two", new BigDecimal("30.00"));
    createTestItem("TEST-003", "Item Three", new BigDecimal("20.00"));

    // When & Then - Sort by basePrice descending
    mockMvc.perform(get("/api/v1/items")
        ...
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data.content", hasSize(3)))
        .andExpect(jsonPath("$.data.content[0].basePrice").value(30.00))
        .andExpect(jsonPath("$.data.content[1].basePrice").value(20.00))
        .andExpect(jsonPath("$.data.content[2].basePrice").value(10.00));
  }
